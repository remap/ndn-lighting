...
#            self.log = BeaconLogging("Beacon-beta", "IColorFlex", "IColorFlex.log", logging.WARNING,logging.WARNING, None)
    ...
    def _dmxpayload(self):
        ...
        if (self._payloadDirty):
			...
			print "************** here ********", p, k, self._payload, self._payload[p][3*k], self._payload[p][3*k+1], self._payload[p][3*k+2]
                else:
                    for k in xrange(self._STRANDSIZE):
...
#	def setRGB(self, r, g, b, k, channels=None):
#        if channels==None:
#            channels = range(0,self._size)
#		if (k=='*'):
#			for k in channels:
...
if __name__ == '__main__':  
	...
	print "provide iColorFlex controller IP as arguments"
	sys.exit(1)
	...
	print("**************** data received... ****************");
	# data = 1/2(iport#) | * | r | g | b
	datafields = data.strip().split('|')
	iport = int(datafields[0])
#	if (datafields[1] == '*'):
	ir = int(datafields[2])
	ig = int(datafields[3])
	ib = int(datafields[4])
	...
	print ib
        ...
        Z = IColorFlex(2)
        K = KinetSenderV2.KinetSender("127.0.0.1", icolorIP, 2, 50, logging.getLogger('KinetSender'))
	print "********** after KinetSenderV2 ******"
...
#	    N2-=1
	    Z.setRGB(ir, ig, ib)
#            Z.setRGB(ir, ig, ib, [istring])
#	    Z.setRGB(N1, N1, N1, [0])
#	    Z.setRGB(N2, N2, N2, [1])
            K.setPayload(iport, Z.payload[0]) # Port 1, K.setPayload(1, ...); Port 2, K.setPayload(2, ...)
            print str(K.getPayload(iport)) # getPayload of Port iport, 1 or 2
            time.sleep(2)
            print "***** RGB sent... ***"
