...
# this has been modified to drive color blaze in 16 bit RGBAW mode
#
class ColorBlast(object):
	...
	#			 1			12
	# ColorBlast(NUM_PORTS, NUM_LIGHTS, logging.getLogger('ColorBlast'))
    def __init__(self, ports, lights, log):    
        ...
        self._payload = [(c_ubyte * (10 * self._lights[k]))() for k in range(self._ports)]      
    ...
    def _dmxpayload(self):
        ...
        if (self._payloadDirty):
            ...
            for p in xrange(self._ports):
                for k in xrange(self._lights[p]):
                    self._payload[p][5*k] = self.ch[p][k].r
                    self._payload[p][5*k+1] = self.ch[p][k].g 
                    self._payload[p][5*k+2] = self.ch[p][k].b
                    self._payload[p][5*k+3] = self.ch[p][k].a 
                    self._payload[p][5*k+4] = self.ch[p][k].w
            self._payloadDirty = False
        return self._payload
    ...
    def setRGBAW(self, r, g, b, a, w, ports=None, channels=None):
		if ports==None:
			ports = range(self._ports)
		for p in ports:
			if channels==None:
			...
			for k in channels:
				...
				if g is not None: self.ch[p][k].g = g
				if b is not None: self.ch[p][k].b = b
				if a is not None: self.ch[p][k].a = a
				if w is not None: self.ch[p][k].w = w
				self.log.info('Set port='+str(p)+',light='+str(k)+':'+str(r)+','+str(g)+','+str(b)+','+str(a)+','+str(w))
			self._payloadDirty = True
