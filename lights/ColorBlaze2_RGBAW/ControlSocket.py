...
class ControlSocket(Thread):
    def __init__(self, cport=DEFAULT_PORT, start=True):
		...
		logging.basicConfig(filename="ColorBlast.log", filemode='w', level=logging.INFO)
		...
		self._kinetSender = KinetSender.KinetSender(SRC_IP, DATA_ENABLER_IP, NUM_PORTS, 10*NUM_LIGHTS, logging.getLogger('KinetSender'))
				                      #__init__(self,   srcip, destip,   fixtureports, fixtureLength, log, start=True):
		if start:
			self.start()
   ...
   # def _processLight(self, id, r, g, b):
    def _processLight(self, id, r, g, b, a, w):
		(port, ch) = _lightMap(id)
		if port==0 or ch==0: return 0
		self._colorBlast.setRGB(r, g, b, a, w, [port-1], [ch-1])
		return port

    def _processCmd(self, cmd):
        # Command format:
        # NUM LIGHTS | ID1 | R | G | B | ID2 | R | ...
		# NUM LIGHTS | ID1 | R | G | B | A | W | ID2 | R | ...
	    ...
	    print("********* in _processCmd ************")
            ...
            for i in range(num_lights):
				try:
					...
					a = int(cfields[4*i+5])
					w = int(cfields[4*i+6])
					ports_to_update.append(self._processLight(id, r, g, b, a, w))
				except Exception, e:
