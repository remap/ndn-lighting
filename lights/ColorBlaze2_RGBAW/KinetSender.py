...
class KinetSender(Thread):
    ...
    #KinetSender(SRC_IP, DATA_ENABLER_IP, NUM_PORTS, 10*NUM_LIGHTS, logging.getLogger('KinetSender'))
    ...
    def run(self):
        ...
        while(not self.stop):
            ...
            time.sleep(self.PACKET_INTERVAL_SECONDS)
            #try:  
...
class KinetPktPortOut(LittleEndianStructure):
    ...
    MAX_PAYLOAD = 511   # ColorBlast PowerCore specific!!
    START_CODE = 0x00 # ColorBlast PowerCore: Non-ChromASIC light
	...
	self.port	= 0x00
	self.flags	= 0x00
	self.timerval	= 0x0000
        self.universe   = 0xFFFFFFFF
        self.startcode  = self.START_CODE
        self.touched    = False
    def __str__(self):
        return "KinetPktPortOut{ len(payload)=" + str(len(self.payload))+"; payload=" + str([str(D) for D in self.payload]) + "; }"
