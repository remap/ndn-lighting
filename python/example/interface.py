...
class controller(pyccn.Closure):
	...
	def upcall(self, kind, info):
		...
		self.parseAndSendToDMX(info.Interest.name)	
	...
	def parseAndSendToDMX(self,name):
	    iMax = len(name)
	    rgbVal = str(name[iMax-(self.appCfg.deviceNameDepth-4)])  # would be -2 but we're not using namecrypto
	    command = str(name[iMax-(self.appCfg.deviceNameDepth-3)]) # would be -1 but we're not using namecrypto
