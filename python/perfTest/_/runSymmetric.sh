...
# just to get sudo privileges for script
sudo ls
...
python logger_server.py events_symmetric.log &

# 2
# run packet capture on controller host, pre-filter for interface host
echo " starting network packet capture..."
sudo /usr/sbin/tcpdump host 131.179.141.19 -w network_symmetric.pcap -s 5000 -Nf &

# 3 - lighting interface
echo "\nrunning lighting interface..."
# ssh INTERFACE_HOST, run:
ssh tv1 "cd /home/lighting/ndn-lighting/python/perfTest/ ; /usr/bin/python -m cProfile -o interface_symm.profile interface_symm.py interface_cfg " &
#ssh tv1 /home/lighting/ndn-lighting/python/perfTest/runInterfaceSymm.sh &

sleep 3 #give ssh time to start listening on gumstix

# 4 - lighting controller
echo "\nrunning lighting controller..."
# on CONTROLLER HOST, run:
cd /home/lighting/ndn-lighting/python/perfTest/
/usr/bin/python -m cProfile -o control_symm.profile controller_symm.py controller_cfg &
