...
python logger_server.py events_unsigned.log &

# 2
# run packet capture on controller host, pre-filter for interface host
echo " starting network packet capture..."
sudo /usr/sbin/tcpdump host 131.179.141.19 -w network_unsigned.pcap -s 5000 -Nf &

# 3 - lighting interface
echo "\nrunning lighting interface..."
# ssh INTERFACE_HOST, run:
ssh tv1 cd /home/lighting/ndn-lighting/python/perfTest/; /usr/bin/python -m cProfile -o interface_unsigned.profile interface_unsigned.py interface_cfg &
...
/usr/bin/python -m cProfile -o control_unsigned.profile controller_unsigned.py controller_cfg &
