...
if __name__ == '__main__':
    ...
    alist = CheckList.scan_files("/home/bupt/ndn-test/mu-lighting/music-file2",fpostfix=".mp3")
    for p in alist:
    	s = p[:-4]  
	...
	p = PutFile(Name(i), Name("ndn:/example/repo/1"), sFace,sLoop)
    	p.insertStream = open(str("music-file2/")+musicFile, 'rb')
    	p.fileSize = os.stat(str("music-file2/")+musicFile).st_size    
    	p.start()
	...
	of = PutFile(Name(i+str("-o")), Name("ndn:/example/repo/1"), sFace,sLoop)
    	of.insertStream = open(str("music-file2/")+osFile, 'rb')
    	of.fileSize = os.stat(str("music-file2/")+osFile).st_size    
    	of.start()

	freqFile = i+str(".txt")
	fq = PutFile(Name(i+str("-f")), Name("ndn:/example/repo/1"),sFace,sLoop)
    	fq.insertStream = open(str("music-file2/")+freqFile, 'rb')
    	fq.fileSize = os.stat(str("music-file2/")+freqFile).st_size    
    	fq.start()
